...
from src.configuration import Configuration, init_config
from src.offline_loading import load_from_local_files, load_from_website
from src.online_query import query
...
def main():
    """
    Main entry point for the src CLI.
    ...
    if "--query" in sys.argv or "--load" in sys.argv:
        ...
        print("Please use:")
        print("  src query <your_query> --max_iter 3")
        print(
            "  src load <your_local_path_or_url> --collection_name <your_collection_name> --collection_desc <your_collection_description>"
        )
        sys.exit(1)
    ...
    parser = argparse.ArgumentParser(prog="src", description="Deep Searcher.")
